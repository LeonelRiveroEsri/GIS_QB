...
export function buildSeriesSummaries(rows: DetailRow[]): SeriesSummary[] {
  ...
  rows.forEach((row) => {
    ...
    // Use an empty sensor segment for collar so IDs keep the same 3-part format
    const keyCollar = `${pozo}||COTA_COLLAR`;

    if (!map.has(keyElevacion)) {
      map.set(keyElevacion, []);
    }

    map.get(keyElevacion)?.push(row);

    if (row.COTA_COLLAR !== null && row.COTA_COLLAR !== undefined) {
      if (!map.has(keyCollar)) {
        map.set(keyCollar, []);
      }
  ...
  map.forEach((items, key) => {
    ...
    const fechaUltimoRegistro = formatDate(last.FECHA ?? last.FECHA_HORA);
