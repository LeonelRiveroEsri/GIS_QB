...
import type { ProfileDeltaInfo, ProfileTooltipItem } from './profile-chart'
...
interface IState {
  ...
  tooltipItems: ProfileTooltipItem[]
  deltaInfo: ProfileDeltaInfo
  comparisonBaseLayerId: string
  comparisonTargetLayerId: string
  slopeLayerId: string
}
...
export default class ResultPane extends React.PureComponent<Props, IState> {
  ...
  constructor (props) {
    ...
    const comparisonBaseLayerId = this.props.addedElelvationProfileLayers?.[0]?.id || ''
    const comparisonTargetLayerId = this.props.addedElelvationProfileLayers?.[1]?.id || comparisonBaseLayerId
    const slopeLayerId = comparisonTargetLayerId || comparisonBaseLayerId
    this.state = {
      ...
      tooltipItems: [],
      deltaInfo: null,
      comparisonBaseLayerId,
      comparisonTargetLayerId,
      slopeLayerId
    }
  ...
  componentDidUpdate = (prevProps: Props) => {
    ...
    if (prevProps.commonDsGeneralSettings.showLegend !== this.props.commonDsGeneralSettings?.showLegend ||
      ...
      this.didElevationLayersInfoChanged(prevProps.addedElelvationProfileLayers, this.props.addedElelvationProfileLayers) ||
      ...
      const runtimeElevationLayers = this.props.addedElelvationProfileLayers || []
      const isDisplayStats = runtimeElevationLayers.filter((layerInfo) => layerInfo.displayStatistics)?.[0]?.displayStatistics ||
      ...
      this.setState({
        showLegend: this.props.commonDsGeneralSettings?.showLegend,
        displayStats: isDisplayStats,
        addedElevationLayers: runtimeElevationLayers,
        comparisonBaseLayerId: this.getValidComparisonLayerId(this.state.comparisonBaseLayerId, runtimeElevationLayers, 0),
        comparisonTargetLayerId: this.getValidComparisonLayerId(this.state.comparisonTargetLayerId, runtimeElevationLayers, 1),
        slopeLayerId: this.getValidComparisonLayerId(this.state.slopeLayerId, runtimeElevationLayers, 1)
      }, () => {
  ...
  getValidComparisonLayerId = (layerId: string, layers: ElevationLayersInfo[], fallbackIndex: number): string => {
    if (layers?.some((layer) => layer.id === layerId)) {
      return layerId
    }
    return layers?.[fallbackIndex]?.id || layers?.[0]?.id || ''
  }

  getComparisonLayerOptions = () => {
    return this.state.addedElevationLayers?.map((layer) => ({
      value: layer.id,
      label: layer.label
    })) || []
  }

  onComparisonBaseChange = (evt) => {
    const nextBaseLayerId = evt.target.value
    let nextTargetLayerId = this.state.comparisonTargetLayerId
    if (nextBaseLayerId === nextTargetLayerId) {
      nextTargetLayerId = this.getComparisonLayerOptions().find((layer) => layer.value !== nextBaseLayerId)?.value || nextTargetLayerId
    }
    this.setState({
      comparisonBaseLayerId: nextBaseLayerId,
      comparisonTargetLayerId: nextTargetLayerId,
      deltaInfo: null,
      tooltipItems: []
    })
  }

  onComparisonTargetChange = (evt) => {
    const nextTargetLayerId = evt.target.value
    let nextBaseLayerId = this.state.comparisonBaseLayerId
    if (nextTargetLayerId === nextBaseLayerId) {
      nextBaseLayerId = this.getComparisonLayerOptions().find((layer) => layer.value !== nextTargetLayerId)?.value || nextBaseLayerId
    }
    this.setState({
      comparisonBaseLayerId: nextBaseLayerId,
      comparisonTargetLayerId: nextTargetLayerId,
  ...
  onSlopeLayerChange = (evt) => {
    this.setState({
      slopeLayerId: evt.target.value
    })
  }

  onTooltipContentChange = (tooltipItems: ProfileTooltipItem[]) => {
    this.setState({
      tooltipItems
    })
  }

  onDeltaInfoChange = (deltaInfo: ProfileDeltaInfo | null) => {
    this.setState({
      deltaInfo
    })
  }

  getMarkerStyle = (tooltipItem: ProfileTooltipItem): React.CSSProperties => {
    const markerStyle: React.CSSProperties = {}
    if (tooltipItem.color) {
      markerStyle.backgroundColor = tooltipItem.color
    }
    if (tooltipItem.markerStyle) {
      tooltipItem.markerStyle.split(';').forEach((declaration) => {
        const [property, value] = declaration.split(':')
        if (!property || !value) {
          return
        }
        const key = property.trim().replace(/-([a-z])/g, (_match, letter) => letter.toUpperCase())
        ;(markerStyle as any)[key] = value.trim()
      })
    }
    return markerStyle
  }
  ...
  renderTooltipHeader = () => {
    const comparisonOptions = this.getComparisonLayerOptions()
    if (comparisonOptions.length < 2) {
      return null
    }

    return (
      <div className='profile-tooltip-header' aria-live='polite'>
        <div className='profile-delta-box' title={this.state.deltaInfo ? `${this.state.deltaInfo.targetLabel} - ${this.state.deltaInfo.baseLabel}` : ''}>
          <span className='profile-delta-label'>ΔH</span>
          <strong>{this.state.deltaInfo?.value || '--'}</strong>
        </div>
        <div className='profile-compare-selectors'>
          <Select aria-label='Superficie base' size='sm' value={this.state.comparisonBaseLayerId} onChange={this.onComparisonBaseChange}>
            {comparisonOptions.map((layer) => {
              return <Option role='option' aria-label={layer.label} key={layer.value} value={layer.value}>{layer.label}</Option>
            })}
          </Select>
          <Select aria-label='Superficie a comparar' size='sm' value={this.state.comparisonTargetLayerId} onChange={this.onComparisonTargetChange}>
            {comparisonOptions.map((layer) => {
              return <Option role='option' aria-label={layer.label} key={layer.value} value={layer.value}>{layer.label}</Option>
            })}
          </Select>
          <Select aria-label='DEM para etiquetas de pendiente' title='DEM para etiquetas de pendiente' size='sm' value={this.state.slopeLayerId} onChange={this.onSlopeLayerChange}>
            {comparisonOptions.map((layer) => {
              return <Option role='option' aria-label={layer.label} key={layer.value} value={layer.value}>Pendiente: {layer.label}</Option>
            })}
          </Select>
        </div>
      </div>
  ...
  render () {
    ...
    const tooltipHeader = this.renderTooltipHeader()

    if (!this.props.chartRender && !this.state.noFeaturesError && !this.state.noValidInput) {
      return <div className={'ep-drawing-compact'} css={getContainerStyle(this.props.theme)}>
        <div className='ep-drawing-message' tabIndex={0} ref={this._alertElement} aria-label={infoMessagesForSelectDraw}>
          <Alert className={'w-100 userInfo py-1'}
            open
        ...
        <div className='ep-drawing-actions'>
          ...
          {this.props.commonDsGeneralSettings?.buttonStyle === ButtonTriggerType.IconText &&
            <Button role={'button'} aria-label={this.nls('doneButtonLabel')} title={this.nls('doneButtonLabel')}
              className={'m-1 actionButton text-break'} size={'default'} type='primary' onClick={this.onDoneClick}>
              <React.Fragment>
    ...
    return <div className={'h-100 w-100'} css={getContainerStyle(this.props.theme)}>
      <div style={{ height: 58 }} className={'ep-widget-header d-flex w-100'}>
        <div className={'align-items-center w-100 pt-1'} style={{ display: 'inline-block' }}>
          {tooltipHeader}
        ...
        <div className={classNames('w-100 h-100', this.props.chartRender ? '' : 'alignInfo align-items-center', this.props.noFeaturesFoundError ? 'alignInfo align-items-center' : '')}>
          ...
          {(this.props.chartRender || this.state.exportData) &&
            <ProfileChart
              ...
              onTooltipContentChange={this.onTooltipContentChange}
              onDeltaInfoChange={this.onDeltaInfoChange}
              comparisonLayerIds={[this.state.comparisonBaseLayerId, this.state.comparisonTargetLayerId]}
              slopeLayerId={this.state.slopeLayerId}
              ...
              slopeLabelLayer={this.props.slopeLabelLayer}
              profileInputGeometry={this.props.profileInputGeometry}
