...
export interface ProfileTooltipItem {
  label: string
  value: string
  color?: string
  markerStyle?: string
...
export interface ProfileDeltaInfo {
  value: string
  baseLabel: string
  targetLabel: string
  rawValue: number
...
interface Props {
  ...
  slopeLabelLayer: GraphicsLayer
  profileInputGeometry: __esri.Geometry
  mapView: JimuMapView
  ...
  onTooltipContentChange?: (items: ProfileTooltipItem[]) => void
  onDeltaInfoChange?: (deltaInfo: ProfileDeltaInfo | null) => void
...
export default class Chart extends React.PureComponent<Props, IState> {
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (profileSettingsChanged || intersectingSettingsChanged || prevProps.assetIntersectionResult !== this.props.assetIntersectionResult ||
      ...
      !lodash.isDeepEqual(prevProps.comparisonLayerIds, this.props.comparisonLayerIds) ||
      prevProps.slopeLayerId !== this.props.slopeLayerId ||
          ...
          profileOutput.forEach((chartData, pointIndex) => {
            ...
            this._elevationLayersOutput?.forEach((profileOutput) => {
              ...
              if (profileOutput.id !== profileResult.lines[0].id) {
                // DEM services can return different sample counts. Resolve by distance
                // instead of assuming that every surface has the same array index.
                elevationInfo[profileOutput.layerId] = this.getElevationAtDistance(profileOutput.samples, elevationInfo.x)
              }
            })
  ...
  getChartPadding = (params) => {
    ...
    const hasDeltaAxisLabels = this.getComparisonLayerIds().length === 2
    return { paddingTop: 0, paddingRight: r, paddingBottom: hasDeltaAxisLabels ? Math.max(b, 28) : b, paddingLeft: l }
  }
  ...
  configureCursorAM5 = ({ chart, xAxis, yAxis, params }) => {
    ...
    chartTooltip.set('forceHidden', true)
  ...
  initChart = async (params) => {
    ...
    let groundSeries
    const comparisonLayerIds = this.getComparisonLayerIds()
    //Create and add all the elevation layers serieses
    this._elevationLayersOutput?.forEach((profileInfo) => {
      if (!this.isComparisonLayerVisible(profileInfo.id, comparisonLayerIds)) {
        return
      }
  ...
  showChartTooltip = (chartInfo) => {
    ...
    if (nearestElevationData) {
      ...
      let innerHtml = ''
      const tooltipItems: ProfileTooltipItem[] = []
      ...
      this._elevationLayersOutput?.forEach((profileInfo) => {
        ...
        if (profileInfo.id === this.props.profileResult.lines[0].id) {
          ...
          if (groundSeries.isVisible() && nearestElevationData.y !== null) {
            ...
            tooltipItems.push({
              label: this._chartSeries.get('name'),
              value: formattedY,
              color: this._chartSeries.get('stroke').toString()
            })
          }
        } else {
          if (profileInfo.series.isVisible() && nearestElevationData[profileInfo.layerId] !== null) {
            ...
            tooltipItems.push({
              label: profileInfo.profileStyles.label,
              value: formattedProfileLineY,
              color: profileInfo.profileStyles.style.lineColor
            })
          }
        }
      })
      ...
      if (this.props.profileResult.lines.length > 0 && this.props.showVolumetricObj && this.props.mapView.view.type === '3d') {
        if (this._volumetricObjLineSeries.isVisible()) {
          ...
          tooltipItems.push({
            label: this._volumetricObjLineSeries.get('name'),
            value: formattedLineViewY,
            color: this._volumetricObjLineSeries.get('stroke').toString()
          })
        }
      }
                ...
                if (series.get('valueYField') === seriesInfo.seriesId && series.isVisible() && nearestElevationData[seriesInfo.seriesId] !== undefined) {
                  ...
                  if (!uniqueTooltipForIntersection.includes(uniqueValue)) {
                    ...
                    if (seriesInfo.displayField) {
                      ...
                      tooltipItems.push({
                        label: series.get('name') + ' (' + seriesInfo.displayField + ')',
                        value: formattedSeriesY,
                        markerStyle: seriesInfo.assetStyle
                      })
                    } else {
      ...
      if (innerHtml !== '' && yAxis.get('min') <= maxY) {
        this.props.onTooltipContentChange?.(tooltipItems)
        this.props.onDeltaInfoChange?.(this.getDeltaInfo(nearestElevationData, yAxisNumberFormatter))
  ...
  */
  createChart = async (params: ChartParams) => {
    ...
    cursor.events.on('cursorhidden', () => {
      ...
      this.props.onTooltipContentChange?.([])
      this.props.onDeltaInfoChange?.(null)
      this.hideChartPosition()
    })
  }

  normalizeLayerKey = (value: string = ''): string => {
    return value.replace(/\/$/, '').toLowerCase()
  }

  getProfileLineSamples = (layerId: string): any[] => {
    const layerInfo = this.props.addedElelvationProfileLayers?.find((layer) => layer.id === layerId)
    const layerUrl = this.normalizeLayerKey((layerInfo as any)?.elevationLayerUrl)
    const layerLabel = this.normalizeLayerKey(layerInfo?.label)
    const line = this.props.profileResult?.lines?.find((profileLine) => {
      const lineId = this.normalizeLayerKey(profileLine.id)
      const lineTitle = this.normalizeLayerKey(profileLine.title || profileLine.name || profileLine.label)
      const lineUrl = this.normalizeLayerKey(profileLine.source?.url || profileLine.layer?.url || profileLine.elevationLayerUrl)
      return lineId === this.normalizeLayerKey(layerId) ||
        (!!layerLabel && lineTitle === layerLabel) ||
        (!!layerUrl && lineUrl === layerUrl)
    })
    if (line?.samples?.length) {
      return line.samples
    }
    // Keep the configured menu ID as the source of truth, with the normalized
    // chart output as a fallback for scene-ground profiles.
    return this._elevationLayersOutput?.find((profileLine) => {
      return profileLine.id === layerId ||
        (!!layerLabel && this.normalizeLayerKey(profileLine.profileStyles?.label) === layerLabel)
    })?.samples || []
  }

  getElevationAtDistance = (samples: any[], distance: number): number => {
    if (!samples?.length) {
      return null
    }

    const firstDistance = convertSingle(samples[0].distance, this.props.profileResult.effectiveUnits.distance, this.props.selectedLinearUnit)
    const lastDistance = convertSingle(samples[samples.length - 1].distance, this.props.profileResult.effectiveUnits.distance, this.props.selectedLinearUnit)
    if (distance <= firstDistance) {
      return this.getConvertedSampleElevation(samples[0])
    }
    if (distance >= lastDistance) {
      return this.getConvertedSampleElevation(samples[samples.length - 1])
    }

    for (let index = 1; index < samples.length; index++) {
      const previousSample = samples[index - 1]
      const currentSample = samples[index]
      const previousDistance = convertSingle(previousSample.distance, this.props.profileResult.effectiveUnits.distance, this.props.selectedLinearUnit)
      const currentDistance = convertSingle(currentSample.distance, this.props.profileResult.effectiveUnits.distance, this.props.selectedLinearUnit)
      if (distance >= previousDistance && distance <= currentDistance) {
        const previousElevation = this.getConvertedSampleElevation(previousSample)
        const currentElevation = this.getConvertedSampleElevation(currentSample)
        if (previousElevation === null || currentElevation === null || currentDistance === previousDistance) {
          return previousElevation
        }
        const ratio = (distance - previousDistance) / (currentDistance - previousDistance)
        return previousElevation + ((currentElevation - previousElevation) * ratio)
      }
    }
    return null
  }

  getConvertedSampleElevation = (sample: any): number => {
    if (sample?.elevation === null || sample?.elevation === undefined) {
      return null
    }
    return convertSingle(sample.elevation, this.props.profileResult.effectiveUnits.elevation, this.props.selectedElevationUnit)
  }

  getSlopeSegments = (layerId: string): SlopeSegmentInfo[] => {
    if (!layerId) {
      return []
    }

    const targetSamples = this.getProfileLineSamples(layerId)
    if (!targetSamples.length) {
      return []
    }

    const totalDistance = convertSingle(
      this.props.profileResult.statistics.maxDistance,
      this.props.profileResult.effectiveUnits.distance,
      this.props.selectedLinearUnit
    )
    const slopeSegments: SlopeSegmentInfo[] = []
    let cumulativeDistance = 0
    const profileGeometries: any[] = []
    const inputGeometry: any = this.props.profileInputGeometry
    if (inputGeometry?.type === 'polyline' && inputGeometry.paths?.length) {
      profileGeometries.push(inputGeometry)
    } else {
      this.props.drawingLayer?.graphics?.forEach((drawGraphic) => {
        const geometry: any = drawGraphic.geometry
        if (geometry?.type === 'polyline' && geometry.paths?.length) {
          profileGeometries.push(geometry)
        }
      })
    }

    profileGeometries.forEach((geometry) => {
      geometry.paths.forEach((path) => {
        for (let index = 1; index < path.length; index++) {
          const startVertex = path[index - 1]
          const endVertex = path[index]
          const segment = new Polyline({
            hasZ: geometry.hasZ,
            hasM: geometry.hasM,
            paths: [[lang.clone(startVertex), lang.clone(endVertex)]],
            spatialReference: geometry.spatialReference.toJSON()
          })
          const segmentDistance = this.getLengthAsPerMapProjection(segment, this.props.selectedLinearUnit)
          const startDistance = cumulativeDistance
          const endDistance = cumulativeDistance + segmentDistance
          cumulativeDistance = endDistance

          if (!segmentDistance || segmentDistance <= 0) {
            continue
          }

          const startElevation = this.getElevationAtDistance(targetSamples, startDistance)
          const endElevation = this.getElevationAtDistance(targetSamples, endDistance)
          if (startElevation === null || endElevation === null) {
            continue
          }

          const horizontalDistance = convertSingle(segmentDistance, this.props.selectedLinearUnit, this.props.selectedElevationUnit)
          if (!horizontalDistance || horizontalDistance <= 0) {
            continue
          }

          const slope = ((endElevation - startElevation) / horizontalDistance) * 100
          const label = `${slope > 0 ? '+' : ''}${intl.formatNumber(slope, { maximumFractionDigits: 1 })}%`
          const midpointDistance = (startDistance + endDistance) / 2
          const midpointElevation = (startElevation + endElevation) / 2
          const chartX = this.props.isFlip ? totalDistance - midpointDistance : midpointDistance
          const dx = endVertex[0] - startVertex[0]
          const dy = endVertex[1] - startVertex[1]
          const segmentLengthMapUnits = Math.hypot(dx, dy)
          const labelOffsetMapUnits = Math.min(Math.max(segmentLengthMapUnits * 0.18, 6), 18)
          const normalX = segmentLengthMapUnits > 0 ? -dy / segmentLengthMapUnits : 0
          const normalY = segmentLengthMapUnits > 0 ? dx / segmentLengthMapUnits : 0
          const mapPoint = new Point({
            x: (startVertex[0] + endVertex[0]) / 2,
            y: (startVertex[1] + endVertex[1]) / 2,
            z: startVertex.length > 2 && endVertex.length > 2 ? (startVertex[2] + endVertex[2]) / 2 : undefined,
            spatialReference: geometry.spatialReference.toJSON()
          })
          const labelPoint = new Point({
            x: mapPoint.x + (normalX * labelOffsetMapUnits),
            y: mapPoint.y + (normalY * labelOffsetMapUnits),
            z: mapPoint.z,
            spatialReference: geometry.spatialReference.toJSON()
          })
  ...
  displaySlopeLabels = (ctx) => {
    // Calculate every DEM so switching the selector only changes which labels are displayed.
    const slopeSegmentsByLayer = new Map<string, SlopeSegmentInfo[]>()
    this.props.addedElelvationProfileLayers?.forEach((layer) => {
      slopeSegmentsByLayer.set(layer.id, this.getSlopeSegments(layer.id))
    })
    const selectedLayerId = this.props.slopeLayerId || this.props.addedElelvationProfileLayers?.[1]?.id || this.props.addedElelvationProfileLayers?.[0]?.id
    let slopeSegments = slopeSegmentsByLayer.get(selectedLayerId) || []
    if (!slopeSegments.length) {
      const selectedLayer = this.props.addedElelvationProfileLayers?.find((layer) => layer.id === selectedLayerId)
      const selectedLabel = this.normalizeLayerKey(selectedLayer?.label)
      const selectedUrl = this.normalizeLayerKey((selectedLayer as any)?.elevationLayerUrl)
      const fallbackLayer = this.props.addedElelvationProfileLayers?.find((layer) => {
        return layer.id !== selectedLayerId &&
          (this.normalizeLayerKey(layer.label) === selectedLabel ||
          this.normalizeLayerKey((layer as any).elevationLayerUrl) === selectedUrl)
      })
      slopeSegments = fallbackLayer ? (slopeSegmentsByLayer.get(fallbackLayer.id) || []) : []
    }
    this.props.slopeLabelLayer?.removeAll()
    if (!slopeSegments.length) {
      return
    }

    this.displaySlopeLabelsOnMap(slopeSegments)
  }

  displaySlopeLabelsOnChart = (ctx, slopeSegments: SlopeSegmentInfo[]) => {
    const chartLabelData = slopeSegments.map((segment) => ({
      x: segment.x,
      y: segment.y,
      label: segment.label
    }))
    const slopeSeries = LineSeriesAm5.new(ctx.chart.root, {
      name: 'Pendiente',
      connect: false,
    ...
    slopeSeries.strokes.template.setAll({
      strokeOpacity: 0
    })
    slopeSeries.fills.template.setAll({
      visible: false
    })
    slopeSeries.bullets.push((root, _series, dataItem) => {
      const labelText = (dataItem.dataContext as any).label
      return BulletAm5.new(root, {
        sprite: LabelAm5.new(root, {
          text: labelText,
          centerX: p50,
          centerY: p50,
          dy: -14,
          ...
          paddingBottom: 2,
          paddingLeft: 5,
          fill: this.getColor(this.props.theme.sys.color.surface.paperText),
          background: RectangleAm5.new(root, {
            fill: this.getColor(this.props.theme.sys.color.surface.paper),
            stroke: this.getColor(this.props.theme.sys.color.divider.primary),
            fillOpacity: 1
          })
        })
      })
    })
    slopeSeries.data.setAll(chartLabelData)
    ctx.chart.series.push(slopeSeries)
    this._seriesToBeHiddenInLegend.push(slopeSeries.uid)
  }

  displaySlopeLabelsOnMap = (slopeSegments: SlopeSegmentInfo[]) => {
    const graphics = slopeSegments.map((segment) => {
      return new Graphic({
        geometry: segment.mapPoint,
        symbol: {
          type: 'text',
          text: segment.label,
          color: [0, 0, 0, 220],
          backgroundColor: [255, 255, 255, 205],
          borderLineColor: [90, 90, 90, 120],
          borderLineSize: 1,
        ...
        attributes: {
          esriCTGraphicType: 'slope-label'
        }
      })
    })
    this.props.slopeLabelLayer?.addMany(graphics)
  }

  getComparisonLayerIds = (): string[] => {
    if (this.props.comparisonLayerIds?.length === 2) {
      return this.props.comparisonLayerIds
    }
    return this.props.addedElelvationProfileLayers?.slice(0, 2).map((layer) => layer.id) || []
  }

  isComparisonLayerVisible = (layerId: string, comparisonLayerIds: string[]): boolean => {
    return comparisonLayerIds.length !== 2 || comparisonLayerIds.includes(layerId)
  }

  getComparisonLayerLabel = (layerId: string): string => {
    return this.props.addedElelvationProfileLayers?.find((layer) => layer.id === layerId)?.label || layerId
  }

  getComparisonLayerValue = (layerId: string, distance: number, elevationData: any): number => {
    const samples = this.getProfileLineSamples(layerId)
    const sampleValue = this.getElevationAtDistance(samples, distance)
    if (sampleValue !== null && sampleValue !== undefined && Number.isFinite(sampleValue)) {
      return sampleValue
    }
    // The chart already contains normalized values for every visible DEM. Use
    // them when a scene service exposes a different runtime profile identifier.
    if (layerId === this.props.profileResult?.lines?.[0]?.id && Number.isFinite(elevationData?.y)) {
      return elevationData.y
    }
    const profileOutput = this._elevationLayersOutput?.find((profileInfo) => profileInfo.id === layerId)
    const chartValue = profileOutput ? elevationData?.[profileOutput.layerId] : null
    return Number.isFinite(chartValue) ? chartValue : null
  }

  getDeltaInfo = (elevationData: any, yAxisNumberFormatter: any): ProfileDeltaInfo => {
    const comparisonLayerIds = this.getComparisonLayerIds()
    if (comparisonLayerIds.length !== 2) {
      return null
    }

    const baseLayerId = comparisonLayerIds[0]
    const targetLayerId = comparisonLayerIds[1]
    const pointIndex = elevationData?.pointIdx
    const effectiveDistance = pointIndex !== null && pointIndex !== undefined
      ? this._allDistances?.[pointIndex]
      : null
    if (effectiveDistance === null || effectiveDistance === undefined) {
      return null
    }
    const distance = convertSingle(effectiveDistance, this.props.profileResult.effectiveUnits.distance, this.props.selectedLinearUnit)
    const baseValue = this.getComparisonLayerValue(baseLayerId, distance, elevationData)
    const targetValue = this.getComparisonLayerValue(targetLayerId, distance, elevationData)
    if (baseValue === null || baseValue === undefined || targetValue === null || targetValue === undefined) {
      return null
    }

    const delta = targetValue - baseValue
    const formattedDelta = yAxisNumberFormatter.format(delta, 5)
    return {
      value: delta > 0 ? `+${formattedDelta}` : formattedDelta,
      baseLabel: this.getComparisonLayerLabel(baseLayerId),
      targetLabel: this.getComparisonLayerLabel(targetLayerId),
      rawValue: delta
    }
  }

  getDeltaLabelAtDistance = (distance: number): string => {
    const comparisonLayerIds = this.getComparisonLayerIds()
    if (comparisonLayerIds.length !== 2 || !Number.isFinite(distance)) {
      return ''
    }
    if (distance <= 0) {
      return ''
    }
    const baseValue = this.getComparisonLayerValue(comparisonLayerIds[0], distance, { x: distance })
    const targetValue = this.getComparisonLayerValue(comparisonLayerIds[1], distance, { x: distance })
    if (baseValue === null || baseValue === undefined || targetValue === null || targetValue === undefined) {
      return ''
    }
    const delta = targetValue - baseValue
    const formattedDelta = intl.formatNumber(delta, { maximumFractionDigits: 2 })
  ...
  configureXAxis = (ctx) => {
    ...
    renderer.labels.template.adapters.add('text', (text, target) => {
      ...
      const deltaLabel = this.getDeltaLabelAtDistance(value as number)
      return deltaLabel ? `${text}\n|\n${deltaLabel}` : text
    })
    const updateXAxisLabelDensity = () => {
      const start = xAxis.get('start') ?? 0
      const end = xAxis.get('end') ?? 1
      const visibleRatio = Math.max(0.01, end - start)
      const minGridDistance = visibleRatio < 0.2
        ? Math.max(22, Math.round(theme.xAxisMinGridDistance * 0.45))
        : visibleRatio < 0.5
          ? Math.max(30, Math.round(theme.xAxisMinGridDistance * 0.65))
          : theme.xAxisMinGridDistance
      renderer.set('minGridDistance', minGridDistance)
    }
    xAxis.on('start', updateXAxisLabelDensity)
    xAxis.on('end', updateXAxisLabelDensity)
    // set axis toolitp
