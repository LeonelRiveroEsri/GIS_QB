...
interface IState {
  ...
  floatingPanelMaximized: boolean
  isControllerWindowLayout: boolean
}
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
  ...
  private _slopeLabelLayer: GraphicsLayer
  private _chartHoverLayer: GraphicsLayer
  ...
  private _floatingPanelDragOffset: { x: number, y: number }
  private readonly _floatingPanelRef = React.createRef<HTMLDivElement>()
  private readonly _widgetContainerRef = React.createRef<HTMLDivElement>()
  private _widgetResizeObserver: ResizeObserver
  ...
  constructor (props) {
    ...
    this._floatingPanelDragOffset = null
    this.state = {
      initialStage: true,
      resultStage: false,
      ...
      floatingPanelX: 24,
      floatingPanelY: 24,
      floatingPanelWidth: 980,
      floatingPanelHeight: 610,
      floatingPanelMaximized: false,
      isControllerWindowLayout: false
    }
  }
  ...
  createDrawingLayers = () => {
    ...
    this._slopeLabelLayer = new GraphicsLayer({
      listMode: 'hide',
      elevationInfo: {
    ...
    this._chartHoverLayer = new GraphicsLayer({
      listMode: 'hide'
    })
  }
  ...
  componentDidMount = async () => {
    ...
    this._widgetResizeObserver = new ResizeObserver(() => {
      this.updateControllerWindowLayout()
    })
    if (this._widgetContainerRef.current) {
      this._widgetResizeObserver.observe(this._widgetContainerRef.current)
      this.updateControllerWindowLayout()
    }
    this.setState({
      noFeaturesError: false,
  ...
  updateControllerWindowLayout = () => {
    const width = this._widgetContainerRef.current?.getBoundingClientRect().width || window.innerWidth
    const isControllerWindowLayout = this.props.browserSizeMode === BrowserSizeMode.Small || width <= 520
    if (isControllerWindowLayout !== this.state.isControllerWindowLayout) {
      this.setState({
        isControllerWindowLayout,
        floatingPanelCollapsed: false,
        floatingPanelMaximized: false
      })
    }
    ...
    if (jmv) {
      this.setState({
        ...
        setTimeout(() => {
          ...
          this.state.jimuMapView.view.map.addMany([this._bufferLayer, this._nextPossibleSelectionLayer, this._drawingLayer, this._intersectionHighlightLayer, this._slopeLabelLayer, this._chartHoverLayer])
          this.createApiWidget(jmv)
          const sceneElevationLayers = this.getSceneElevationLayers(jmv)
          if (jmv.view.type === '3d') {
            this.setState({
              addedElevationLayers: sceneElevationLayers,
              groundLayerId: sceneElevationLayers[0]?.id || ''
            }, () => {
  ...
  checkLineLayerAvailableInDsAndConfig = (activeDs: string) => {
    ...
    if (this.state.lineLayersNotFound !== noLineLayerFound) {
      this.setState({
  ...
  setConfigForDataSources = () => {
    ...
    const sceneElevationLayers = this.getSceneElevationLayers(this.state.jimuMapView)
    const useSceneElevationLayers = this.state.jimuMapView?.view?.type === '3d'
    const configuredElevationLayers = configActiveDs ? configActiveDs.elevationLayersSettings?.addedElevationLayers : this._defaultConfig.elevationLayersSettings?.addedElevationLayers
    const elevationLayers = useSceneElevationLayers ? sceneElevationLayers : configuredElevationLayers
    const groundLayer = useSceneElevationLayers
      ? (sceneElevationLayers[0]?.id || '')
      : (configActiveDs ? configActiveDs.elevationLayersSettings?.groundLayerId : this._defaultConfig.elevationLayersSettings?.groundLayerId)
    const nextState = {
      addedElevationLayers: elevationLayers,
    ...
    if (!lodash.isDeepEqual({
      addedElevationLayers: this.state.addedElevationLayers,
      groundLayerId: this.state.groundLayerId,
      graphicsHighlightColor: this.state.graphicsHighlightColor,
      selectionModeOptions: this.state.selectionModeOptions,
  ...
  getSceneElevationLayers = (jmv: JimuMapView): ElevationLayersInfo[] => {
    if (jmv?.view?.type !== '3d') {
      return []
    }
    const sceneGroundLayers: any[] = jmv.view.map?.ground?.layers?.toArray?.() || []
    const configDs = this.props.config.configInfo[this.state.currentDatasource]
    const configuredLayers: ElevationLayersInfo[] = configDs?.elevationLayersSettings?.addedElevationLayers || []
    const knownLayers = configuredLayers.concat(this.state.addedElevationLayers || [])
    const fallbackColors = ['#b54900', '#007ac2', '#56a01a', '#a849b3', '#d17e00', '#008c95']
    const normalizeUrl = (url: string = '') => url.replace(/\/$/, '').toLowerCase()

    const isEsriTerrain = (layer): boolean => {
      const title = (layer.title || '').toLowerCase()
      const url = (layer.url || '').toLowerCase()
      const portalItemId = (layer.portalItem?.id || '').toLowerCase()
      return title.includes('terrain 3d') ||
        title.includes('terrain3d') ||
        title.includes('world elevation') ||
        url.includes('elevation3d.arcgis.com') ||
        url.includes('/terrain3d/') ||
        portalItemId === '58a541efc59545e6b7137f961d7de883'
    }
    const orderedGroundLayers = sceneGroundLayers
      .map((layer, originalIndex) => ({ layer, originalIndex }))
      .sort((a, b) => Number(isEsriTerrain(a.layer)) - Number(isEsriTerrain(b.layer)) || a.originalIndex - b.originalIndex)
      .map(({ layer }) => layer)

    return orderedGroundLayers.map((sceneLayer, index) => {
      const sceneUrl = normalizeUrl(sceneLayer.url)
      const configuredLayer = knownLayers.find((layer) => {
        return normalizeUrl(layer.elevationLayerUrl) === sceneUrl ||
          layer.useDataSource?.dataSourceId?.endsWith(`-${sceneLayer.id}`)
      })
      return {
        id: configuredLayer?.id || `scene-dem-${sceneLayer.id}`,
        useDataSource: configuredLayer?.useDataSource || null,
        label: sceneLayer.title || configuredLayer?.label || `DEM ${index + 1}`,
        elevationLayerUrl: sceneLayer.url,
        style: configuredLayer?.style || {
          lineType: 'solid-line',
          lineColor: fallbackColors[index % fallbackColors.length],
          lineThickness: 2
        },
        displayStatistics: configuredLayer?.displayStatistics ?? true,
        selectedStatistics: configuredLayer?.selectedStatistics || epStatistics
      }
    })
  }

  onFloatingPanelDragStart = (evt: React.MouseEvent<HTMLDivElement>) => {
    if ((evt.target as HTMLElement)?.closest('button')) {
      return
    }
    const parentRect = (this._floatingPanelRef.current?.offsetParent as HTMLElement)?.getBoundingClientRect()
    this._floatingPanelDragOffset = {
      x: evt.clientX - (parentRect?.left || 0) - this.state.floatingPanelX,
      y: evt.clientY - (parentRect?.top || 0) - this.state.floatingPanelY
    }
    document.addEventListener('mousemove', this.onFloatingPanelDragMove)
    document.addEventListener('mouseup', this.onFloatingPanelDragEnd)
    evt.preventDefault()
  }

  onFloatingPanelDragMove = (evt: MouseEvent) => {
    if (!this._floatingPanelDragOffset) {
      return
    }
    const parentRect = (this._floatingPanelRef.current?.offsetParent as HTMLElement)?.getBoundingClientRect()
    const panel = this._floatingPanelRef.current
    const maxX = Math.max(0, (parentRect?.width || window.innerWidth) - (panel?.offsetWidth || this.state.floatingPanelWidth))
    const maxY = Math.max(0, (parentRect?.height || window.innerHeight) - (panel?.offsetHeight || this.state.floatingPanelHeight))
    const nextX = Math.min(maxX, Math.max(0, evt.clientX - (parentRect?.left || 0) - this._floatingPanelDragOffset.x))
    const nextY = Math.min(maxY, Math.max(0, evt.clientY - (parentRect?.top || 0) - this._floatingPanelDragOffset.y))
    this.setState({
      floatingPanelX: nextX,
      floatingPanelY: nextY
    })
  }

  onFloatingPanelDragEnd = () => {
    this._floatingPanelDragOffset = null
    document.removeEventListener('mousemove', this.onFloatingPanelDragMove)
    document.removeEventListener('mouseup', this.onFloatingPanelDragEnd)
  }

  toggleFloatingPanelCollapsed = () => {
    this.setState({
  ...
  toggleFloatingPanelMaximized = () => {
    this.setState({
      floatingPanelMaximized: !this.state.floatingPanelMaximized,
      floatingPanelCollapsed: false
    })
  }

  onFloatingPanelResizeCommit = () => {
    const panel = this._floatingPanelRef.current
    if (!panel || this.state.floatingPanelCollapsed) {
      return
    }
    const nextWidth = Math.max(520, Math.round(panel.offsetWidth))
    const nextHeight = Math.max(360, Math.round(panel.offsetHeight))
    if (nextWidth !== this.state.floatingPanelWidth || nextHeight !== this.state.floatingPanelHeight) {
      this.setState({
        floatingPanelWidth: nextWidth,
        floatingPanelHeight: nextHeight
      })
    }
  }
  ...
  createEpViewModel = (jmv, layerInfoUpdated?: boolean) => {
    ...
    const getElevationSource = (layerInfo: ElevationLayersInfo) => {
      if (jmv.view.type === '3d') {
        const normalizeUrl = (url: string = '') => url.replace(/\/$/, '').toLowerCase()
        const sceneLayers: any[] = jmv.view.map?.ground?.layers?.toArray?.() || []
        const sceneLayer = sceneLayers.find((layer) =>
          normalizeUrl(layer.url) === normalizeUrl(layerInfo.elevationLayerUrl) ||
          layer.title === layerInfo.label)
        if (sceneLayer) {
          return sceneLayer
        }
      }
      return new ElevationLayer({ url: layerInfo.elevationLayerUrl })
    }

    // Match the native Elevation Profile widget: in a 3D scene the reference
    // profile must use the scene ground. Querying its first ElevationLayer
    // directly can return different resolution/offset values than the composed
    // ground used by the SceneView.
    ...
    normalProfileLayers?.forEach((layerInfo) => {
      profiles.push({
        id: layerInfo.id,
        type: 'query', // displays elevation values from a custom source
        source: getElevationSource(layerInfo),
        viewVisualizationEnabled: false,
    ...
    return new Promise((resolve) => {
      ...
      if (this._defaultViewModel?.input) {
        ...
        if (this._selectedBufferValues.enabled && this._selectedBufferValues.bufferDistance > 0) {
          let inputGeometry = this._defaultViewModel.input.geometry.clone ? this._defaultViewModel.input.geometry.clone() as __esri.GeometryUnion : this._defaultViewModel.input.geometry
  ...
  */
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (prevProps.useMapWidgetIds !== this.props.useMapWidgetIds ||
      ...
      if (this.props.config.configInfo[this.props.config.activeDataSource]) {
        this.setState({
          ...
          this.setState({
            addedElevationLayers: this._mapView?.view?.type === '3d'
              ? this.getSceneElevationLayers(this._mapView)
              : this.props.config.configInfo[this.state.currentDatasource].elevationLayersSettings?.addedElevationLayers
          })
        })
      }
    }
    ...
    if (prevConfig?.elevationLayersSettings?.groundLayerId !== currentConfig?.elevationLayersSettings?.groundLayerId) {
      const activeElevationLayers = this._mapView?.view?.type === '3d'
        ? this.getSceneElevationLayers(this._mapView)
        : currentConfig?.elevationLayersSettings?.addedElevationLayers
      this.setState({
        groundLayerId: this._mapView?.view?.type === '3d'
          ? (activeElevationLayers?.[0]?.id || '')
          : currentConfig?.elevationLayersSettings?.groundLayerId,
        addedElevationLayers: activeElevationLayers
      }, () => {
  ...
  */
  ...
  destroyDrawingLayers = () => {
    ...
    if (this._slopeLabelLayer) {
      this._slopeLabelLayer.removeAll()
      this._slopeLabelLayer.destroy()
    }
    if (this._chartHoverLayer) {
      this._chartHoverLayer.removeAll()
      this._chartHoverLayer.destroy()
    }
    this.hideChartPosition()
  }
  ...
  highlightChartPosition = (position: number) => {
    if (this._defaultViewModel) {
      this._defaultViewModel.hoveredChartPosition = null
    }
    this._chartHoverLayer?.removeAll()
    const geometry: any = this._defaultViewModel?.input?.geometry ||
      this._drawingLayer?.graphics?.find((graphic) => graphic.geometry?.type === 'polyline')?.geometry
    if (!geometry?.paths?.length || position === null || position === undefined) {
      return
    }
    ...
    const targetLength = Math.max(0, Math.min(1, position)) * totalLength
    let traversed = 0
    const segment = segments.find((item) => {
      if (traversed + item.length >= targetLength) return true
    ...
    if (this.state.jimuMapView?.view?.map && this._chartHoverLayer) {
      this.state.jimuMapView.view.map.reorder(this._chartHoverLayer, this.state.jimuMapView.view.map.layers.length - 1)
    }
  }
  ...
  renderProfileStartPane = () => {
    const activeDsConfigInfo = this.props.config.configInfo[this.state.currentDatasource]
    const isProfileSettingsEnabled = !!(activeDsConfigInfo?.advanceOptions || activeDsConfigInfo?.profileSettings?.isProfileSettingsEnabled)
    const canSelectLine = this.state.updatedSelectableLayersAtRuntime.length > 0 && isProfileSettingsEnabled

    return (
      <div className='ep-profile-start'>
        <div className='ep-profile-start-card'>
          <div className='ep-profile-start-title'>{this.nls('drawProfileLabel')}</div>
          <div className='ep-profile-start-desc'>{this.nls('drawProfileDesc')}</div>
          <div className='ep-profile-start-actions'>
            <Button role='button' aria-label={this.nls('drawButtonLabel')} title={this.nls('drawButtonLabel')}
              size='default' type='primary' className='text-break' onClick={this.onDrawButtonClicked}>
              <Icon size={16} icon={epIcon.drawIcon} />
              {this.nls('drawButtonLabel')}
            </Button>
            {canSelectLine &&
              <Button role='button' aria-label={this.nls('selectButtonLabel')} title={this.nls('selectButtonLabel')}
                size='default' type='secondary' className='text-break' onClick={this.onSelectButtonClicked}>
                <Icon size={16} icon={epIcon.selectIcon} />
                {this.nls('selectButtonLabel')}
              </Button>
            }
          </div>
  ...
  renderResultPane = () => {
    return (
      <div
        ref={this._floatingPanelRef}
        className={classNames(
          'ep-floating-panel',
          this.state.isControllerWindowLayout ? 'is-controller-window' : '',
          this.state.initialStage ? 'is-start' : '',
          (!this.state.initialStage && !this.state.startChartRendering) ? 'is-drawing' : '',
          this.state.floatingPanelCollapsed ? 'is-collapsed' : '',
          this.state.floatingPanelMaximized ? 'is-maximized' : ''
        )}
        onMouseUp={this.onFloatingPanelResizeCommit}
      >
        <button
          type='button'
          className='ep-floating-panel-grip'
          title={this.state.floatingPanelCollapsed ? 'Expandir panel' : 'Colapsar panel'}
          onClick={this.toggleFloatingPanelCollapsed}
        />
        <div className='ep-floating-panel-titlebar'>
          <button
            type='button'
            className='ep-floating-panel-title'
            onClick={this.toggleFloatingPanelCollapsed}
            title={this.state.floatingPanelCollapsed ? 'Expandir panel' : 'Colapsar panel'}
          >
            <span>{this.state.floatingPanelCollapsed ? '︿' : '﹀'}</span>
            <span>Perfil de elevación</span>
          </button>
          <div className='ep-floating-panel-actions'>
            <Button
              role='button'
              aria-label={this.state.floatingPanelMaximized ? 'Restaurar panel' : 'Maximizar panel'}
              title={this.state.floatingPanelMaximized ? 'Restaurar panel' : 'Maximizar panel'}
              icon
              size='sm'
              type='tertiary'
              className='ep-floating-panel-toggle'
              onClick={this.toggleFloatingPanelMaximized}
            >
              {this.state.floatingPanelMaximized ? '▣' : '□'}
            </Button>
            <Button
              role='button'
              aria-label={this.state.floatingPanelCollapsed ? 'Expandir panel' : 'Colapsar panel'}
              title={this.state.floatingPanelCollapsed ? 'Expandir panel' : 'Colapsar panel'}
              icon
              size='sm'
              type='tertiary'
              ...
              onClick={this.toggleFloatingPanelCollapsed}
            >
              {this.state.floatingPanelCollapsed ? '+' : '-'}
            </Button>
          </div>
        </div>
        {!this.state.floatingPanelCollapsed &&
          <div className='ep-floating-panel-content'>
            {this.state.initialStage
              ? this.renderProfileStartPane()
              : <ResultPane
                  ...
                  slopeLabelLayer={this._slopeLabelLayer}
                  profileInputGeometry={this._defaultViewModel?.input?.geometry}
  ...
  render () {
    ...
    return (
      <Paper css={getStyle(this.props.theme)} shape='none' className='jimu-widget ep-root-paper'>
        <div ref={this._widgetContainerRef} className={classNames('widget-elevation-profile', this.state.isControllerWindowLayout ? 'is-controller-window' : '')}>
          ...
          {!this.state.onWidgetLoadShowLoadingIndicator && (this.state.initialStage || this.state.resultStage) &&
