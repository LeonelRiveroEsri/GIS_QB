...
async function getUserToken(): Promise<string> {
  ...
  if (!session) {
    throw new Error(
      "No se pudo iniciar sesión en ArcGIS para consultar la capa de pozos.",
    );
  }
  ...
  if (!token) {
    throw new Error(
      "No se pudo obtener token para consultar la capa de pozos.",
    );
  }
...
export async function queryPozosByDetailRows(
  ...
  while (true) {
    ...
    const response = await fetch(
      `${POZOS_LAYER_URL}/query?${params.toString()}`,
    );
